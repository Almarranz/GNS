...
dmax_sirius = 3.  ; max HAWK-I pixel distance between SIRIUS and HAWK-I for calibration star selection
                 ; 1 pixel = 0.053" 
dmax_vvv = 3.    ; max HAWK-I pixel distance between VVV and HAWK-I
...
; correct mean offset between VVV and HAWK-I
;x_shift = median(x_vvv[subc1] - x_scaled[subc2])
;y_shift = median(y_vvv[subc1] - y_scaled[subc2])
;x_sigma = stddev(x_vvv[subc1] - x_scaled[subc2])
;y_sigma = stddev(y_vvv[subc1] - y_scaled[subc2])
;print, 'x_off:  ' + strn(x_shift) + ' +- ' + strn(x_sigma) + ', y_off: ' + strn(y_shift) + ' +- ' + strn(y_sigma)
;x_scaled = x_scaled + x_shift
;y_scaled = y_scaled + y_shift


; find common stars again
;compare_lists, x_vvv, y_vvv, x_scaled, y_scaled, x1c, y1c, x2c, y2c, MAX_DISTANCE=dmax_vvv,$
;   SUBSCRIPTS_1=subc1, SUBSCRIPTS_2 = subc2, SUB1 = sub1, SUB2 = sub2   
...
astro = SOLVE_ASTRO(a_vvv, d_vvv, x[subc2], y[subc2], success=success)
...
dra_tmp =  sin(d_vvv/180.) * (a_vvv - a_hawki) * 3600.
ddec_tmp = (d_vvv - d_hawki) * 3600.
set_plot,'PS',/interpolate
device, XOFFSET=0, YOFFSET=0, $
...
device, XOFFSET=0, YOFFSET=0, $
    ...
    cgplot, m, sm,  PSYM=4, Color = 'black', XTITLE='[' + band + strn(chip) +  ']', YTITLE='[d' + band + ']',$
      THICK = 0.5, XRANGE = [12,22], YRANGE = [0,0.1], SYMSIZE=0.2
device, /close 
...
device, XOFFSET=0, YOFFSET=0, $
    ...
    cgplot, m, sx,  PSYM=4, Color = 'black', XTITLE='[' + band + ', Chip ' + strn(chip) +  ']', YTITLE='dpos, x-axis [mas]',$
            THICK = 0.5, XRANGE = [11,22], YRANGE = [0,0.02], SYMSIZE=0.2
;    nsteps = 18
