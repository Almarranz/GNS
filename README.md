...
#### NOTE: I have changed the order and placed selectgood.pro before dejitter.pro. This make sense only in the case where the first cube contains bad frame that can affect to the long exposure frame, which is the one correl_optimaze proccess is using for the fine alignment
